...
def write_freqs(spec_data, fname, num):
    '''
...
def main():
    ...
    while True:
        ...
        # Get binary data
        data = stream.read(CHUNK) # ,exception_on_overflow = False; add this arg for RASPBERRY
        time.sleep(0.1)
        ...
        if len(maxs) > 0:
            counter = write_freqs(spec_data, fname, counter)
